...
def ipv4_head(raw_data):version_header_length,tos,total_length,identification,flags,ttl,proto,checksum,src,target=struct.unpack('!BBHHHBBH4s4s',raw_data[:20]);version=f"{version_header_length:08b}"[:4];header_length=f"{version_header_length:08b}"[4:];s='.'.join((str(int(x))for x in src));d='.'.join((str(int(x))for x in target));data=raw_data[20:];networkLayer={'version':int(version,2),'header length':int(header_length,2),'tos (type of service)':tos,'total length':total_length,'identification':identification,_J:flags,'ttl (time to live)':ttl,'protocol':_C.format(proto,protocol_map[proto]),_E:_C.format(hex(checksum),checksum),'source address':s,'destination address':d};return networkLayer,data,proto,s,d
def transport_parse(proto,raw_data):
	B='destination port';A='source port'
	if proto==1:typ,code,checksum,identifier,seq=struct.unpack('!BBHHH',raw_data[:8]);data=raw_data[8:];parsed={_G:typ,'code':code,_E:_C.format(hex(checksum),checksum),'identifier':identifier,'sequence number':seq}
	if proto==99999:typ,res,checksum,group,seq,qic,source=struct.unpack('!BBH4sBBH',raw_data[:12]);data=raw_data[12:];parsed={_G:f"{typ:8b}"[:3],'type1':f"{typ:8b}"[3:4],'type2':f"{typ:8b}"[4:],'max res. time':res,_E:_C.format(hex(checksum),checksum),'group address':'.'.join((str(int(x))for x in group)),'seq':seq,'QQIC':qic,'number of sources':source}
	elif proto==17:src_port,dest_port,length,checksum=struct.unpack('!HHHH',raw_data[:8]);data=raw_data[8:];parsed={A:src_port,B:dest_port,'length':length,_E:_C.format(hex(checksum),checksum)}
	elif proto==6:src_port,dest_port,seq,ack,data_flag,ws,checksum,pointer=struct.unpack('!HHIIHHHH',raw_data[:20]);data=raw_data[20:];off=f"{data_flag:16b}"[:4];res=f"{data_flag:16b}"[4:7];flags=f"{data_flag:16b}"[7:];parsed={A:src_port,B:dest_port,'sequece number':seq,'acknowledge number':ack,'data offset':int(off,2),'reserved':res,_J:flags,'windows size':ws,_E:_C.format(hex(checksum),checksum),'urgent pointer':pointer}
	else:return _H
	return parsed,data
...
parser.add_argument('-d','--Dump',help='Dump the packets to a file instead of displaying',default=_H,nargs='?',const=_N)
parser.add_argument('-od','--OpenDump',help='Open and read the dump file',default=_H,nargs='?',const=_N)
...
if args.OpenDump:createsrcstr(source,PCAP_SRC_FILE,_F,_F,args.OpenDump.encode(_D),errbuf);fp=open(source,65536,PCAP_OPENFLAG_PROMISCUOUS,20,_F,errbuf)
